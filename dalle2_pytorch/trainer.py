...
class DiffusionPriorTrainer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.optimizer = get_optimizer(
            self.diffusion_prior.parameters(), **self.optim_kwargs, **kwargs
        )

        if exists(cosine_decay_max_steps):
            self.scheduler = CosineAnnealingLR(
                self.optimizer, T_max=cosine_decay_max_steps
            )
        else:
            self.scheduler = LambdaLR(self.optimizer, lr_lambda=lambda _: 1.0)

        self.warmup_scheduler = (
            warmup.LinearWarmup(self.optimizer, warmup_period=warmup_steps)
        ...
        if self.use_ema:
            self.ema_diffusion_prior = EMA(
                self.accelerator.unwrap_model(self.diffusion_prior), **ema_kwargs
            )
    ...
    def load(self, path_or_state, overwrite_lr=True, strict=True):
        ...
        self.accelerator.unwrap_model(self.diffusion_prior).load_state_dict(
            loaded_obj["model"], strict=strict
        )
        self.step.copy_(
            torch.ones_like(self.step, device=self.device)
            * loaded_obj["step"].to(self.device)
        )
    ...
    def update(self):
        if exists(self.max_grad_norm):
            self.accelerator.clip_grad_norm_(
                self.diffusion_prior.parameters(), self.max_grad_norm
            )
    ...
    def p_sample_loop(self, *args, **kwargs):
        model = (
            self.ema_diffusion_prior.ema_model if self.use_ema else self.diffusion_prior
        )
    ...
    def embed_text(self, *args, **kwargs):
        return self.accelerator.unwrap_model(self.diffusion_prior).clip.embed_text(
            *args, **kwargs
        )
