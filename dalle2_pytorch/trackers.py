...
class WandbLogger(BaseLogger):
    ...
    def init(self, full_config: BaseModel, extra_config: dict, **kwargs) -> None:
        ...
        ), "wandb_entity must be specified for wandb logger"
        assert (
            self.project is not None
        ), "wandb_project must be specified for wandb logger"
        self.wandb = import_or_print_error(
            "wandb", "`pip install wandb` to use the wandb logger"
        )
        ...
        if self.resume:
            ...
            ), "`wandb_run_id` must be provided if `wandb_resume` is True"
            if self.run_name is not None:
    ...
    def log_images(self, images, captions=[], image_section="images", **kwargs) -> None:
        ...
        wandb_images = [
            ...
            for image, caption in zip_longest(images, captions)
        ]
...
def create_logger(logger_type: str, data_path: str, **kwargs) -> BaseLogger:
    if logger_type == "custom":
        raise NotImplementedError(
            "Custom loggers are not supported yet. Please use a different logger type."
        )
    try:
        logger_class = logger_type_map[logger_type]
    except KeyError:
        raise ValueError(
            f"Unknown logger type: {logger_type}. Must be one of {list(logger_type_map.keys())}"
        )
...
class WandbLoader(BaseLoader):
    ...
    def init(self, logger: BaseLogger, **kwargs) -> None:
        self.wandb = import_or_print_error(
            "wandb", "`pip install wandb` to use the wandb recall function"
        )
        ...
        if self.wandb.run is not None and self.run_path is None:
            ...
            assert (
                self.run_path is not None
            ), "wandb run was not found to load from. If not using the wandb logger must specify the `wandb_run_path`."
        assert (
            self.run_path is not None
        ), "`wandb_run_path` must be provided for the wandb loader"
        assert (
            self.file_path is not None
        ), "`wandb_file_path` must be provided for the wandb loader"
...
def create_loader(loader_type: str, data_path: str, **kwargs) -> BaseLoader:
    if loader_type == "custom":
        raise NotImplementedError(
            "Custom loaders are not supported yet. Please use a different loader type."
        )
    try:
        loader_class = loader_type_map[loader_type]
    except KeyError:
        raise ValueError(
            f"Unknown loader type: {loader_type}. Must be one of {list(loader_type_map.keys())}"
        )
...
class BaseSaver:
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            self.saving_latest or self.saving_best or self.saving_meta
        ), "At least one saving option must be specified"

    def init(self, logger: BaseLogger, **kwargs) -> None:
        raise NotImplementedError

    def save_file(
        self, local_path: Path, save_path: str, is_best=False, is_latest=False, **kwargs
    ) -> None:
        """
...
class LocalSaver(BaseSaver):
    ...
    def save_file(self, local_path: str, save_path: str, **kwargs) -> None:
        ...
        print(
            f"Saving {save_path_file_name} {self.save_type} to local path {save_path}"
        )
...
class WandbSaver(BaseSaver):
    ...
    def init(self, logger: BaseLogger, **kwargs) -> None:
        ...
        if self.run_path is not None:
            ...
            assert (
                self.wandb.run is not None
            ), "You must be using the wandb logger if you are saving to wandb and have not set `wandb_run_path`"
    ...
    def save_file(self, local_path: Path, save_path: str, **kwargs) -> None:
        ...
        save_path_file_name = Path(save_path).name
        print(
            f"Saving {save_path_file_name} {self.save_type} to wandb run {self.run.path}-{self.run.name}"
        )
...
class HuggingfaceSaver(BaseSaver):
    ...
    def init(self, logger: BaseLogger, **kwargs):
        ...
        self.hub = import_or_print_error(
            "huggingface_hub",
    ...
    def save_file(self, local_path: Path, save_path: str, **kwargs) -> None:
        ...
        save_path_file_name = Path(save_path).name
        print(
            f"Saving {save_path_file_name} {self.save_type} to huggingface repo {self.huggingface_repo}"
        )
...
def create_saver(saver_type: str, data_path: str, **kwargs) -> BaseSaver:
    if saver_type == "custom":
        raise NotImplementedError(
            "Custom savers are not supported yet. Please use a different saver type."
        )
    try:
        saver_class = saver_type_map[saver_type]
    except KeyError:
        raise ValueError(
            f"Unknown saver type: {saver_type}. Must be one of {list(saver_type_map.keys())}"
        )
...
class Tracker:
    def __init__(
        ...
        if not dummy_mode:
            if not overwrite_data_path:
                ...
                ), f"Data path {self.data_path} already exists. Set overwrite_data_path to True to overwrite."
                if not self.data_path.exists():
                    self.data_path.mkdir(parents=True)
    ...
    def _load_auto_resume(self) -> bool:
        ...
        if not self.auto_resume_path.exists():
            if self.logger.auto_resume:
                print(
                    "Auto_resume is enabled but no auto_resume.json file exists. Assuming this is the first run."
                )
            return False
        ...
        if not self.logger.auto_resume:
            print(
                f"Removing auto_resume.json because auto_resume is not enabled in the config"
            )
    ...
    def init(self, full_config: BaseModel, extra_config: dict):
        ...
        self.blacklisted_checkpoint_metadata_keys = [
            "scaler",
            "optimizer",
            "model",
            "version",
            "step",
            "steps",
        ]  # These keys would cause us to error if we try to save them as metadata
    ...
    def add_save_metadata(self, state_dict_key: str, metadata: Any):
        """
        Adds a new piece of metadata that will be saved along with the model.
        """
        self.save_metadata[state_dict_key] = metadata

    def _save_state_dict(
        self, trainer: DiffusionPriorTrainer, save_type: str, file_path: str, **kwargs
    ) -> Path:
        """
        ...
        if save_type == "checkpoint":
            ...
            metadata = {
                k: v
                for k, v in self.save_metadata.items()
                if k not in self.blacklisted_checkpoint_metadata_keys
            }
            ...
            state_dict = {**self.save_metadata, "model": model_state_dict}
    ...
    def save(self, trainer, is_best: bool, is_latest: bool, **kwargs):
        ...
        for saver in self.savers:
            local_path = (
                checkpoint_path if saver.save_type == "checkpoint" else model_path
            )
            if saver.saving_latest and is_latest:
                latest_checkpoint_path = saver.save_latest_to.format(**kwargs)
                try:
                    saver.save_file(
                        local_path, latest_checkpoint_path, is_latest=True, **kwargs
                    )
            ...
            if saver.saving_best and is_best:
                ...
                try:
                    saver.save_file(
                        local_path, best_checkpoint_path, is_best=True, **kwargs
                    )
                except Exception as e:
                    self.logger.log_error(f"Error saving checkpoint: {e}", **kwargs)
                    print(f"Error saving checkpoint: {e}")
    ...
    def can_recall(self):
        ...
        return self.loader is not None and (
            not self.loader.only_auto_resume or self.did_auto_resume
        )
    ...
    def recall(self):
        if self.can_recall:
            ...
            raise ValueError(
                "Tried to recall, but no loader was set or auto-resume was not performed."
            )
